...
def get_average_photon_number(U, NMax):
    print("\tStarting Average Photonic Excitation.")
...
def get_average_matter_number(U, NMax):
    print("\tStarting Average Matter Excitation.")
...
def get_population_of_matter_states(U, NMax):
    print("\tStarting Matter Population.")
    matter_population = np.zeros(( len(U) ))
    j0_indices = [ j for j in range( len(U) ) if j%(params.nf) == 0 ] # Pick out the |j,0> basis states
    for p in range( NMax ): # Only compute up to the NMax^{th} polaritonic state
        pop = np.conjugate(U[:,p]) * U[:,p]
        matter_population[p] = np.sum( pop[j0_indices] )
    return matter_population

def get_population_of_fock_states(U, NMax):
    print("\tStarting Matter Population.")
    fock_population = np.zeros(( len(U) ))
    zerom_indices = [ j for j in range( len(U) ) if j%(params.nf) != 0 ] # Pick out the |0,m> basis states
    for p in range( NMax ): # Only compute up to the NMax^{th} polaritonic state
        pop = np.conjugate(U[:,p]) * U[:,p]
        fock_population[p] = np.sum( pop[zerom_indices] )
    return fock_population
...
def SolvePlotandSave(Hpol,Had,d,params):
        ...
        np.savetxt( f"data_polariton/Matter_Population_E{d}_eta{params.η_1}_wc{params.ωc}_Nad{params.Nad}.dat", get_population_of_matter_states(U,50) ) # Photonic Number -- Saves Space Compared to Storing all U
        np.savetxt( f"data_polariton/Photon_Population_E{d}_eta{params.η_1}_wc{params.ωc}_Nad{params.Nad}.dat", get_population_of_fock_states(U,50) ) # Photonic Number -- Saves Space Compared to Storing all U
...
def main_Serial():
    ...
    for d in ['x']: # CHOOSE POLARIZATION DIRECTION OF DIPOLE MOMENT
