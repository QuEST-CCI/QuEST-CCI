...
def getGlobals():
    ...
    global EVEC_INTS, EVEC_NORM, EVEC_OUT, RPA, THETA, PHI
...
def get_TD_fast_1r( Upol, TD_matter, NPolCompute ):
    ...
    sp.call(f"rm data_TD/transition_density_contributions_THETA_{THETA}_PHI_{PHI}_WC{WC}.dat",shell=True)
    ...
    for A0IND in range(NA0):
        ...
        for a in range( NPolCompute ):                       # Choose only up to what is needed..
            ...
            sp.call(f'''echo "Non-zero contributions to transition Density for state P{a} for A0 = {round(A0_LIST[A0IND],3)}:" >> data_TD/transition_density_contributions_THETA_{THETA}_PHI_{PHI}_WC{WC}.dat''',shell=True)
            sp.call('''echo "n, j, k, C_{jn}^p, C_{kn}^p, C_{jn}^p * C_{kn}^p"''' + f">> data_TD/transition_density_contributions_THETA_{THETA}_PHI_{PHI}_WC{WC}.dat",shell=True)
                    ...
                    for state_k in range( NM ):           # Matter
                        if ( state_j == state_k or state_j == 0 or state_k == 0 ): # We only have 0-->k and k-->k densities !!!!
                            ...
                            if ( Upol[A0IND,polIND1,0] * Upol[A0IND,polIND2,a] > 1e-3 ):
                                ...
                                sp.call(f"echo {out} >> data_TD/transition_density_contributions_THETA_{THETA}_PHI_{PHI}_WC{WC}.dat",shell=True)
...
def get_diag_density_fast_1r( Upol, TD_matter, NPolCompute ):
    ...
    sp.call(f"rm data_diagonal_density/diagonal_density_contributions_THETA_{THETA}_PHI_{PHI}_WC{WC}.dat",shell=True)
    sp.call(f"touch data_diagonal_density/diagonal_density_contributions_THETA_{THETA}_PHI_{PHI}_WC{WC}.dat",shell=True)
    ...
    for A0IND in range(NA0):
        ...
        for a in range( NPolCompute ):                       # Choose only up to what is needed..
            ...
            sp.call(f'''echo "Non-zero contributions to diagonal Density for state P{a} for A0 = {round(A0_LIST[A0IND],3)}:" >> data_diagonal_density/diagonal_density_contributions_THETA_{THETA}_PHI_{PHI}_WC{WC}.dat''',shell=True)
            sp.call('''echo "n, j, k, C_{jn}, C_{kn}, P_{jn,kn}"''' + f">> data_diagonal_density/diagonal_density_contributions_THETA_{THETA}_PHI_{PHI}_WC{WC}.dat",shell=True)
            for n in range( NF ):                         # Photons
                for state_j in range( NM ):               # Matter
                    ...
                    for state_k in range( NM ):           # Matter
                        ...
                        if ( state_j == state_k or state_j == 0 or state_k == 0 ): # We only have 0-->k and k-->k densities !!!!
                            ...
                            if ( Upol[A0IND,polIND1,a] * Upol[A0IND,polIND2,a] > 1e-3 ):
                                ...
                                sp.call(f"echo {out} >> data_diagonal_density/diagonal_density_contributions_THETA_{THETA}_PHI_{PHI}_WC{WC}.dat",shell=True)
                            ...
                            if ( Upol[A0IND,polIND1,a] * Upol[A0IND,polIND2,a] > 0.01 ): # Tell if missing > 1%
                                out = "\t".join(map(str,[n,state_j,state_k,round(Upol[A0IND,polIND1,a],4), round(Upol[A0IND,polIND2,a],4), round(Upol[A0IND,polIND1,a] * Upol[A0IND,polIND2,a],4)]))
                                sp.call(f"echo ' MISSING: {out}' >> data_diagonal_density/diagonal_density_contributions_THETA_{THETA}_PHI_{PHI}_WC{WC}.dat",shell=True)
                                print( "MISSING!!!",n,state_j,state_k,round(Upol[A0IND,polIND1,a],4), round(Upol[A0IND,polIND2,a],4), round(Upol[A0IND,polIND1,a] * Upol[A0IND,polIND2,a],4) )
...
def plot_dipole( MU ):
    ...
    for c,name in enumerate(["Total","POL"]):

        if ( name == "Total" ):
            MU_d = np.einsum("JKd,d->JK", MU[:,:,:], np.array([1.0,1.0,1.0]) / np.sqrt(3))
        elif ( name == "POL" ):
            MU_d = np.einsum( "JKd,d->JK", MU[:,:,:], EVEC_NORM )
...
def get_HadMU():
    ...
    for j in range( NA0 ):
        ...
        Epol[j,:] = np.loadtxt(f"{data_PF}/E_THETA_{THETA}_PHI_{PHI}_A0_{round(A0,6)}_WC_{round(WC,6)}_NF_{NF}_NM_{NM}.dat")
        #Upol[j,:,:] = np.loadtxt(f"{data_PF}/U_THETA_{THETA}_PHI_{PHI}_A0_{round(A0,6)}_WC_{round(WC,6)}_NF_{NF}_NM_{NM}.dat")
        Upol[j,:,:] = np.load(f"{data_PF}/U_THETA_{THETA}_PHI_{PHI}_A0_{round(A0,6)}_WC_{round(WC,6)}_NF_{NF}_NM_{NM}.dat.npy")
...
def getExansion(Upol):
    ...
    for pol in range( NPolCompute ):
        ...
        for A0IND, A0 in enumerate(A0_LIST):
            A0 = round(A0,6)
            np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_JK_THETA_{THETA}_PHI_{PHI}_A0{A0}_WC{WC}_NM{NM}_NF{NF}.dat", U0_MATTER[:,:,A0IND], fmt="%1.5f" )
            np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_JK_THETA_{THETA}_PHI_{PHI}_A0{A0}_WC{WC}_NM{NM}_NF{NF}_ABS.dat", np.abs(U0_MATTER[:,:,A0IND]), fmt="%1.5f" )
            ...
            plt.savefig(f"data_expansion/RHO_{pol}{pol}_MATTER_JK_THETA_{THETA}_PHI_{PHI}_A0{A0}_WC{WC}_NM{NM}_NF{NF}.jpg",dpi=600)
            plt.clf()

        ROW_MATTER  = U0_MATTER[0,:,:]
        DIAG_MATTER = np.array([ np.array(U0_MATTER[J,J,:]) for J in range( NM ) ])
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_JJ_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.dat", DIAG_MATTER, fmt="%1.5f" )
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.dat", ROW_MATTER, fmt="%1.5f" )
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}_ABS.dat", np.abs(ROW_MATTER), fmt="%1.5f" )
        # Plot diagonal ones
        states = []
        for j in range( NM ):
        ...
        plt.ylim(0)
        plt.savefig(f"data_expansion/RHO_{pol}{pol}_MATTER_JJ_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.jpg",dpi=600)
        plt.clf()
        ...
        plt.xlim(A0_LIST[0], A0_LIST[-1])
        plt.ylim(0)
        plt.savefig(f"data_expansion/RHO_{pol}{pol}_MATTER_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.jpg",dpi=600)
        plt.clf()
        ...
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_PHOTON_JJ_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.dat", DIAG_PHOTON, fmt="%1.5f" )
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_PHOTON_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.dat", np.abs(ROW_PHOTON), fmt="%1.5f" )
        # Plot diagonal ones
        states = []
        for j in range( NF ):
        ...
        plt.xlim(A0_LIST[0], A0_LIST[-1])
        plt.ylim(0)
        plt.savefig(f"data_expansion/RHO_{pol}{pol}_PHOTON_JJ_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.jpg",dpi=600)
        plt.clf()
        # Plot transition ones
        states = []
        for j in range( NF ):
        ...
        plt.xlim(A0_LIST[0], A0_LIST[-1])
        plt.ylim(0)
        plt.savefig(f"data_expansion/RHO_{pol}{pol}_PHOTON_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.jpg",dpi=600)
        plt.clf()
        ...
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_JJ_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}_A0.dat", output_JJ.T, fmt="%1.5f", header=" ".join(map(str,states_JJ)) )
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}_A0.dat", output_0J.T, fmt="%1.5f", header=" ".join(map(str,states_0J)) )
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}_A0_ABS.dat", np.abs(output_0J).T, fmt="%1.5f", header=" ".join(map(str,states_0J)) )
        ...
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_PHOTON_JJ_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}_A0.dat", output_JJ.T, fmt="%1.5f", header=" ".join(map(str,states)) )
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_PHOTON_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}_A0.dat", np.abs(output_0J).T, fmt="%1.5f", header=" ".join(map(str,states)) )
        ...
        ROW_MATTER  = U0_MATTER[0,:,:]
        DIAG_MATTER = np.array([ np.array(U0_MATTER[J,J,:]) for J in range( NM ) ])
        np.savetxt( f"data_expansion/RHO_{0}{pol}_MATTER_JJ_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.dat", DIAG_MATTER, fmt="%1.5f" )
        np.savetxt( f"data_expansion/RHO_{0}{pol}_MATTER_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.dat", np.abs(ROW_MATTER), fmt="%1.5f" )
        # Plot diagonal ones
        states = []
        for j in range( NM ):
            if ( np.max(np.abs(DIAG_MATTER[j,:])) > matter_threshold ):
        ...
        plt.xlim(A0_LIST[0], A0_LIST[-1])
        plt.ylim(0)
        plt.savefig(f"data_expansion/RHO_{0}{pol}_MATTER_JJ_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.jpg",dpi=600)
        plt.clf()
        # Plot transition ones
        states = []
        for j in range( NM ):
        ...
        plt.xlim(A0_LIST[0], A0_LIST[-1])
        plt.ylim(0)
        plt.savefig(f"data_expansion/RHO_{0}{pol}_MATTER_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.jpg",dpi=600)
        plt.clf()
        ...
        ROW_PHOTON  = U0_PHOTON[0,:,:]
        DIAG_PHOTON = np.array([ np.array(U0_PHOTON[J,J,:]) for J in range( NF ) ])
        np.savetxt( f"data_expansion/RHO_{0}{pol}_PHOTON_JJ_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.dat", DIAG_PHOTON, fmt="%1.5f" )
        np.savetxt( f"data_expansion/RHO_{0}{pol}_PHOTON_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.dat", np.abs(ROW_PHOTON), fmt="%1.5f" )
        # Plot diagonal ones
        states = []
        for j in range( NF ):
            if ( np.max(np.abs(DIAG_PHOTON[j,:])) > photon_threshold ):
        ...
        plt.xlim(A0_LIST[0], A0_LIST[-1])
        plt.ylim(0)
        plt.savefig(f"data_expansion/RHO_{0}{pol}_PHOTON_JJ_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.jpg",dpi=600)
        plt.clf()
        # Plot transition ones
        states = []
        for j in range( NF ):
        ...
        plt.xlim(A0_LIST[0], A0_LIST[-1])
        plt.ylim(0)
        plt.savefig(f"data_expansion/RHO_{0}{pol}_PHOTON_0J_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.jpg",dpi=600)
        plt.clf()
        ...
        for A0IND in range( len(A0_LIST) ):
            for count, state in enumerate(states):
                output_JJ[count, :] = DIAG_PHOTON[state,:]
                output_0J[count, :] = ROW_PHOTON[state,:]
        np.savetxt( f"data_expansion/RHO_{0}{pol}_PHOTON_JJ_A0_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.dat", output_JJ.T, fmt="%1.5f", header=" ".join(map(str,states)) )
        np.savetxt( f"data_expansion/RHO_{0}{pol}_PHOTON_0J_A0_THETA_{THETA}_PHI_{PHI}_A0SCAN_WC{WC}_NM{NM}_NF{NF}.dat", np.abs(output_0J).T, fmt="%1.5f", header=" ".join(map(str,states)) )
...
def getDipole_pow_1(Upol,MU):
    ...
    for A0IND in range(len(A0_LIST)):
        dipFile = open( f'data_dipole/dipole_Polaritons_THETA_{THETA}_PHI_{PHI}_A0{A0_LIST[A0IND]}.dat','w' )
...
def plot_TD( TD_matter, TD_pol ):
    ...
    plt.tight_layout()
    plt.savefig(f"data_TD/Transition_Density_Slices_POL_Matter_X_THETA_{THETA}_PHI_{PHI}.jpg")
    plt.clf()
    ...
    plt.ylabel('Transition Density',fontsize=15)
    plt.tight_layout()
    plt.savefig(f"data_TD/Transition_Density_Slices_POL_Matter_Y_THETA_{THETA}_PHI_{PHI}.jpg")
    plt.clf()
    ...
    plt.ylabel('Transition Density',fontsize=15)
    plt.tight_layout()
    plt.savefig(f"data_TD/Transition_Density_Slices_POL_Matter_Z_THETA_{THETA}_PHI_{PHI}.jpg")
    plt.clf()
    ...
    if ( write_TD_Files == True ):
        ...
        for p in range(NPolCompute): # p = 1 --> T_{01}(R)
            for A0IND in range(len(A0_LIST)):
                ...
                f = open(f'data_TD/trans_THETA_{THETA}_PHI_{PHI}_A0{A0}_WC{WC}_NM{NM}_NF{NF}_P0{p}.cube','w')
...
def compute_Transition_density_matrix_1r( Upol, TDM_matter ):
    ...
    for A0IND in range( len(A0_LIST) ):
        for p in range( NPolCompute ):
            ...
            plt.colorbar(pad=0.01)
            plt.savefig(f"data_TD/TDM_P_0_{p}_THETA_{THETA}_PHI_{PHI}_A0{A0_LIST[A0IND]}.jpg",dpi=300)
            plt.clf()
            ...
            plt.title( r"TDM $P_0$ $\rightarrow$ $P_{}$,   $A_0$ = {}  a.u.".format(p,A0_LIST[A0IND]) ,fontsize=15)
            plt.savefig(f"data_TD/TDM_P_0_{p}_THETA_{THETA}_PHI_{PHI}_A0{A0_LIST[A0IND]}_diag.jpg",dpi=300)
            plt.clf()
        ...
        for p in range(NPolCompute): # p = 1 --> NTO_{01}(R)
            for A0IND in range(len(A0_LIST)):
                for ind,eh in enumerate( ['HOTO','LUTO'] ):
                    ...
                    f = open(f'data_TD/NTO_{eh}_THETA_{THETA}_PHI_{PHI}_A0{A0}_WC{WC}_NM{NM}_NF{NF}_P0{p}.cube','w')
    ...
    if ( write_TD_Files == True ):
        ...
        for p in range(NPolCompute): # p = 1 --> T_{1-1}(R)
            for A0IND in range(len(A0_LIST)):
                ...
                f = open(f'data_diagonal_density/diagonal_density_THETA_{THETA}_PHI_{PHI}_A0{A0}_WC{WC}_NM{NM}_NF{NF}_P{p}{p}.cube','w')
    ...
    if ( write_TD_Files == True ):
        ...
        for p in range(N_OPTIONS):
            for A0IND in range(len(A0_LIST)):
                ...
                f = open(f'data_difference_density/difference_density_{option_names[p]}_THETA_{THETA}_PHI_{PHI}_A0{A0}_WC{WC}_NM{NM}_NF{NF}.cube','w')
            ...
            plt.tight_layout()
            plt.savefig(f'data_difference_density/difference_density_{option_names[p]}_THETA_{THETA}_PHI_{PHI}_WC{WC}_NM{NM}_NF{NF}.jpg',dpi=600)
            plt.clf()
            ...
            for A0IND,A0 in enumerate(A0_LIST):
                ...
                np.savetxt( f'data_difference_density/difference_density_contour_THETA_{THETA}_PHI_{PHI}_{option_names[p]}_A0{A0}_WC{WC}_NM{NM}_NF{NF}.dat', func_interp(RX_fine_CUT,RY_fine_CUT).T )
                ...
                plt.tight_layout()
                plt.savefig(f'data_difference_density/difference_density_contour_THETA_{THETA}_PHI_{PHI}_{option_names[p]}_A0{A0}_WC{WC}_NM{NM}_NF{NF}.jpg',dpi=600)
                plt.clf()
