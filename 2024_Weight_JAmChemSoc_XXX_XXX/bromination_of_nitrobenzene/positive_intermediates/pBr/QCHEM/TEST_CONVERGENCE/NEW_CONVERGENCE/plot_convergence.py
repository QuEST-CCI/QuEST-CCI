...
for A0i,A0 in enumerate(A0_LIST):
    for NMi,NM in enumerate(NM_LIST):
        E_MSCAN[A0i,NMi] = np.loadtxt( f"PF_NF{NF_FIX}_NM{NM}/data_PF/E_THETA_{round(THETA,2)}_PHI_{round(PHI,2)}_A0_{round(A0,6)}_WC_{round(wc_eV,6)}_NF_{NF_FIX}_NM_{NM}.dat" )[0] / 27.2114 * 627.5

    for NFi,NF in enumerate(NF_LIST):
        E_FSCAN[A0i,NFi] = np.loadtxt( f"PF_NF{NF}_NM{NM_FIX}/data_PF/E_THETA_{round(THETA,2)}_PHI_{round(PHI,2)}_A0_{round(A0,6)}_WC_{round(wc_eV,6)}_NF_{NF}_NM_{NM_FIX}.dat" )[0] / 27.2114 * 627.5

### MATTER SCAN ###
for A0i,A0 in enumerate(A0_LIST):
    #plt.plot( NM_LIST, E_MSCAN[A0i] - np.min(E_MSCAN[A0i,0]), "-o", label="$A_0$"+f" = {A0}" )
    plt.plot( NM_LIST, E_MSCAN[A0i] - np.min(E_MSCAN[A0i,-1]), "-o", label="$A_0$"+f" = {A0}" )
plt.legend()
...
for A0i,A0 in enumerate(A0_LIST):
    ...
    plt.semilogx( NF_LIST, E_FSCAN[A0i] - np.min(E_FSCAN[A0i,-1]), "-o", label="$A_0$"+f" = {A0}" )
plt.legend()
