...
def getExansion(Upol):
    ...
    for pol in range( NPolCompute ):
        ...
        for A0IND, A0 in enumerate(A0_LIST):
            ...
            DATA = U0_MATTER[:,:,A0IND]
            DATA[ DATA <= matter_threshold ] = 0.0
            np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_JK_{EVEC_OUT}_A0{A0}_WC{WC}_NM{NM}_NF{NF}.dat", DATA, fmt="%1.5f" )
            plt.imshow( DATA, origin='lower', cmap='brg', norm=mpl.colors.LogNorm(vmin=0.01, vmax=1.00) )
            plt.colorbar(pad=0.01)
            plt.xlabel("Electronic State $\\alpha$",fontsize=15)
