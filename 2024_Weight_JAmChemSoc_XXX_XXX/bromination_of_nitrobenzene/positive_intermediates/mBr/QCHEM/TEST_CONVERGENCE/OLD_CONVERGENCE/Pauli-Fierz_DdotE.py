...
def get_globals():
    ...
    NM        = BASH_NM                  # Number of Electronic States (including ground state)
    NF        = BASH_NF                  # Number of Fock Basis States
...
def get_ADIABATIC_DATA():
    ...
    if ( RPA == True ):
        EAD += np.loadtxt(f"../QCHEM/PLOTS_DATA/ADIABATIC_ENERGIES_RPA.dat")[:NM] # in AU
        MU  += np.load(f"../QCHEM/PLOTS_DATA/DIPOLE_RPA.dat.npy")[:NM,:NM] # in AU
    else:
        EAD += np.loadtxt(f"../QCHEM/PLOTS_DATA/ADIABATIC_ENERGIES_TDA.dat")[:NM] # in AU
        MU  += np.load(f"../QCHEM/PLOTS_DATA/DIPOLE_TDA.dat.npy")[:NM,:NM] # in AU
