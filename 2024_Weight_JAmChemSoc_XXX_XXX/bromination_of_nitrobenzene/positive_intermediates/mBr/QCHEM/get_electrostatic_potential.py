...
def read_density_cube(filename):
    ...
    global NAtoms, NGrid, Nxyz, dLxyz, Lxyz, coords, atom_charges, coords_array, Origin
    header = np.array([ np.array(j.split(),dtype=float) for j in open(filename,"r").readlines()[2:6] ])
    NAtoms = int(header[0,0])
    Origin = np.array([header[0,1],header[0,2],header[0,3] ]).astype(float)
    ...
    print (f'\tTD Grid  = {np.product(Nxyz)}')
...
#@jit(nopython=True)
def compute_electrostatic_potential_BRADEN( CUBE_DATA ):
    """
    ...
    for xi,x in enumerate( XGRID ):
        ...
        for yi,y in enumerate( YGRID ):
            ...
            for zi,z in enumerate( ZGRID ):
                ...
                RINV = 1/R
                RINV[ RINV == float('inf') ] = 0 # 1e8 # Choose big number  WHAT TO DO HERE ?
...
def compute_electrostatic_potential_ChatGPT( CUBE_DATA ):
    # Compute the electric potential
    x, y, z = np.meshgrid(np.arange(Nxyz[0]), np.arange(Nxyz[1]), np.arange(Nxyz[2]), indexing='ij')
    POT = np.zeros( Nxyz )
    for i in range(Nxyz[0]):
        print( i ) 
        for j in range(Nxyz[1]):
            for k in range(Nxyz[2]):
                r = np.sqrt(((k*dLxyz[0] + Origin[0]) - (x*dLxyz[0] + Origin[0]))**2 + \
                            ((j*dLxyz[1] + Origin[1]) - (y*dLxyz[1] + Origin[1]))**2 + \
                            ((i*dLxyz[2] + Origin[2]) - (z*dLxyz[2] + Origin[2]))**2)
                POT[i,j,k] = np.sum(CUBE_DATA / r)
...
def saveCUBE( POT ):
    ...
    f.write(f"{NAtoms} {Origin[0]} {Origin[1]} {Origin[2]}\n")
...
def main():
    ...
    #POT       = compute_electrostatic_potential_BRADEN( CUBE_DATA )
    POT       = compute_electrostatic_potential_ChatGPT( CUBE_DATA )
    saveCUBE( POT )
