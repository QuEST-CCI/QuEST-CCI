...
def get_H_PF(EAD, MU):
    ...
    H_PF  += kron_prod(dMU, q_op) * wc_AU * A0             # Interaction
    H_PF  += kron_prod(dMU @ dMU, I_ph) * wc_AU * A0**2     # Dipole Self-Energy
...
def get_ADIABATIC_DATA():
    ...
    if ( RPA == True ):
        EAD += np.loadtxt(f"../../../mBr/QCHEM/PLOTS_DATA/ADIABATIC_ENERGIES_RPA.dat")[:NM] # in AU
        MU  += np.load(f"../../../mBr/QCHEM/PLOTS_DATA/DIPOLE_RPA.dat.npy")[:NM,:NM] # in AU
    else:
        EAD += np.loadtxt(f"../../../mBr/QCHEM/PLOTS_DATA/ADIABATIC_ENERGIES_TDA.dat")[:NM] # in AU
        MU  += np.load(f"../../../mBr/QCHEM/PLOTS_DATA/DIPOLE_TDA.dat.npy")[:NM,:NM] # in AU
