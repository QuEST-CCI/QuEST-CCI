...
def getExansion(Upol):
    ...
    for pol in range( NPolCompute ):
        ...
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_0J_{EVEC_OUT}_A0SCAN_WC{WC}_NM{NM}_NF{NF}_A0.dat", output_0J.T, fmt="%1.5f", header=" ".join(map(str,states)) )
        np.savetxt( f"data_expansion/RHO_{pol}{pol}_MATTER_0J_{EVEC_OUT}_A0SCAN_WC{WC}_NM{NM}_NF{NF}_A0_ABS.dat", np.abs(output_0J).T, fmt="%1.5f", header=" ".join(map(str,states)) )
...
def plot_diag_contributions():
    ...
    for A0_ind,A0 in enumerate( A0_LIST ):
        for state in range( NPolCompute ):
            ...
            np.savetxt(f"data_diagonal_density/MATTER_CONTRIBUTIONS_JJ_P{state}_{EVEC_OUT}_A0{A0}_WC{WC}.dat", MATTER_CONT[np.diag_indices(len(MATTER_CONT))])
